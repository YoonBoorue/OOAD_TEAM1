...
| Processor layer | `DustProcessor`, `ObstacleProcessor` | dust response와 obstacle avoidance direction 결정. Obstacle decision은 ~~front/left/right sensor input~~ [삭제] front/left sensor input과 `turnRight()` 후 front sensor recheck를 기준으로 한다. [변경] |
...
| `ObstacleProcessor` delegates to `OperatingMode::checkIsMoving()` | obstacle direction 선택 후 mode가 motor output command를 수행한다. right는 sensor input으로 선택하지 않고 `turnRight()` 후 front sensor recheck로 판단하는 설계로 변경한다. [추가] |
...
| `SD_ObstacleAvoidance_ObjectSequence` | `docs/ai-output/sdd_sd_obstacle_avoidance.puml` | UC5, UC12, UC13, UC14 | `obstacleDetected(direction)`, output commands | obstacle decision과 motor command mapping. right sensor input은 제거하고 `turnRight()` 후 front sensor recheck로 right 가능 여부를 판단한다. [추가] |
...
| `obstacleDetected(direction)` | source: project artifact, implementation/test | `Controller::obstacleDetected(const bool direction[3])` | `ObstacleProcessor::decideDirection()`, `OperatingMode::checkIsMoving()`, `MotorDriver` commands | direction 배열은 ~~front/left/right blocked 상태를 의미한다.~~ [삭제] 유지보수 설계 기준으로 front/left sensor input만 의미하며, right blocked 상태는 sensor input이 아니라 `turnRight()` 후 front sensor recheck로 판단한다. [변경] |
...
| `turnRight()` | `obstacleDetected(direction)` | `ObstacleProcessor` chooses `Direction::RIGHT`, mode executes. The choice is based on ~~right sensor input~~ [삭제] front/left input plus `turnRight()` and front sensor recheck. [변경] | `MotorDriver::turnRight()` then front sensor recheck; `moveForward()` when clear [변경] | direction rotates right relative to current direction, then front sensor confirms whether the path is clear [추가] |
| `moveBackward()` | `obstacleDetected(direction)` | `ObstacleProcessor` chooses `Direction::BACK`, mode executes. all-blocked equivalent flow uses right turn -> left turn(front restore) -> backward movement while checking right through front sensor, with left priority for final available direction. [추가] | `MotorDriver::moveBackward()` | forward flag false; moving status remains as previous active movement state |
...
| UC12/UC13/UC14 routed through `obstacleDetected(direction)` | Turn left/right/back are not independent incoming system operations. | Obstacle sequence shows these as output commands selected by `ObstacleProcessor` and mode. right turn remains an output command, but ~~right sensor input remains part of direction~~ [삭제] right availability is confirmed by `turnRight()` followed by front sensor recheck. [변경] |
...
| UC5 Avoid Obstacle | `obstacleDetected(direction)` plus motor output commands. direction input is ~~front/left/right~~ [삭제] front/left sensor input; right is checked by `turnRight()` + front sensor recheck. [변경] | `SD_ObstacleAvoidance_ObjectSequence` | `Controller::obstacleDetected()`, `ObstacleProcessor::decideDirection()`, mode `checkIsMoving()` | `ControllerObstacleDetectedTest.cpp` | `TC24-TC43` | Partial due cleaner-stop mismatch and ~~pending right-sensor-input removal~~ [삭제] |
...
| UC13 Turn Right | Trigger: `obstacleDetected(direction)`; output `turnRight()` and ~~`moveForward()`~~ [삭제] front sensor recheck, then `moveForward()` when clear [변경] | `SD_ObstacleAvoidance_ObjectSequence` | `ObstacleProcessor::decideDirection()`, `MotorDriver::turnRight()`, `MotorDriver::moveForward()`; right sensor input is not required [추가] | `ControllerObstacleDetectedTest.cpp` | `TC30`, `TC36`, `TC40` | Supported by behavior goal; ~~code update pending for right-sensor-input removal~~ [삭제] |
| UC14 Move Backward | Trigger: `obstacleDetected(direction)`; output `moveBackward()` after right-turn/front-recheck path is unavailable [추가] | `SD_ObstacleAvoidance_ObjectSequence` | `ObstacleProcessor::decideDirection()`, `MotorDriver::moveBackward()`; all-blocked equivalent flow restores front with left turn, moves backward while checking right via front sensor, then moves to available direction with left priority [추가] | `ControllerObstacleDetectedTest.cpp` | `TC26`, `TC28`, `TC32-TC33`, `TC36`, `TC38`, `TC42-TC43` | Supported by behavior goal; ~~code update pending for right-sensor-input removal~~ [삭제] |
...
| Right sensor input removal | SRS maintenance change removes ~~right sensor input~~ [삭제] right sensor input while keeping `turnRight()` as an avoidance output command. | SDD design must route right availability through `turnRight()` followed by front sensor recheck; ~~current code evidence may still show legacy `direction[2]` until implementation is updated.~~ [삭제] |
